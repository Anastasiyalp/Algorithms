...
class Bohr
{
  ...
  private:
    ...
    int get_suff_link(int v, int count_rec)
    {
        ...
        if (bohr[v].suff_link == -1)        //если еще не считали
        {
            ...
            for(int i = 0; i < count_rec; i++)
                cout << '\t';
            if (bohr[bohr[v].par].symb == joker) //Если родитель - джокер
            {
                cout << "Родитель джокер. Суфф. ссылкаа ведет к нему." << endl;
                bohr[v].suff_link = bohr[v].par;
            }else if (v == 0 || bohr[v].par == 0) //если v - корень или предок v - корень
            {
            ...
            cout << "суфф. ссылка уже была найдена. Она ведет к "<< (bohr[v].suff_link ? (char)(bohr[bohr[v].suff_link].symb +' ') : '$') << endl;
    ...
    int get_auto_move(int v, char ch)
    {
        if (bohr[v].auto_move[ch] == -1)                                     //Если для нашего случая переход автомата еще не определен, то определим его
        {
            ...
            }else if (bohr[v].next_vrtx[joker] != -1)                        //Или есть джокер
            {
  ...
  public:
    ...
    void find_all_pos(const string &s)
    {
        ...
        {
            cout << endl << i+1<< ") "<< s[i] << " - ";
            u = get_auto_move(u, s[i] - ' '); //Переход автомата
